-- ============================================================
-- Fix dar_compliance() falsely marking DARs as "late" when the
-- actual WhatsApp send time was never captured.
--
-- Root cause: 56.5% of daily_reports rows have message_sent_at =
-- NULL (a dar-consolidate bug where a malformed/duplicate Whapi
-- entry with timestamp 0 could overwrite a good timestamp — fixed
-- separately in supabase/functions/dar-consolidate/index.ts).
-- This RPC then did COALESCE(message_sent_at, submitted_at), where
-- submitted_at is the CRON JOB'S run time (~23:20 IST) — always
-- past the 14:00-next-day cutoff — so any row with an unknown send
-- time was counted as late by default, with zero evidence.
--
-- Fix: only count a day as "late" when message_sent_at is actually
-- known and past cutoff. Unknown timing is treated as on-time
-- (benefit of the doubt for a data-capture gap, not employee
-- behavior). Only this affected CTE changed — rest of the function
      ...
      COUNT(DISTINCT dr.report_date) FILTER (
        WHERE dr.message_sent_at IS NULL
           OR (dr.message_sent_at AT TIME ZONE 'Asia/Kolkata')
              < ((dr.report_date + 1)::timestamp + INTERVAL '14 hours')
